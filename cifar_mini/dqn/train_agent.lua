...
while episode < max_episode do
	...
	local function getIterator(mode)
	   return tnt.ParallelDatasetIterator{
		  ...
		  closure = function()
			 local dataset = provider[mode..'Data']  --由于是闭包，这里的dataset只会定义一次。
			 ...
			 if mode == 'train' then
				return list_dataset  --一次性返回390个batch，共外部的iterator使用。如何使得每个iteration都可以随意变换batch呢？ 下一步计划：调研DarasetIterator()的接口。
				   :transform{
	...
	function step(state, batch_loss, action, tof)
		--take action from DQN, get minibatch
		--TODO
		--[[
				...
				call =
					function(self, network, criterion, iterator, maxepoch, optimMethod,
						...
						while state.epoch < state.maxepoch do
							...
							local dataset = provider['trainData']
							--local simu_action = 1
							for i = 1, 390 do
								local batchindex = minibatch:step(global_action)
								--simu_action = simu_action % 10 + 1
								local bsize = minibatch.batchsize
								local sample = {
									target = torch.LongTensor(bsize),
									input = torch.FloatTensor(bsize, 3, 32, 32)
								}
								for j = 1, bsize do
									sample.input[j]  = hflip(randomcrop(dataset.data[batchindex[j]]:float()))
									sample.target[j] = dataset.labels[batchindex[j]]
								end
								--input = dataset.data[idx]:float(),
								--target = torch.LongTensor{dataset.labels[idx]},
								state.sample = sample
