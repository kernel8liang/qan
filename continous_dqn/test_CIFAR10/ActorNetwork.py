...
class ActorNetwork(object):
    ...
    def create_actor_network(self, state_size, action_dim):
        ...
        # Custom activation for LR
        def custom_activation_lr(x):
            return (K.sigmoid(x) * -12.5) - 2.5

        # Learning rate (LR) from CNN
        lr = Dense(1, activation=custom_activation_lr)(h1)

        # Custom activation for LR
        def custom_activation_spike(x):
            return (K.sigmoid(x) * -3) - 3.9

        spike = Dense(1, activation=custom_activation_spike)(h1)
        v = merge([lr, spike], mode='concat')
