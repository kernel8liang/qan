...
X_batch = x_train[:64]
y_batch = y_train[:64]
hist = model.train_on_batch(X_batch, y_batch)
a = model.evaluate(x_test, y_test, batch_size=32, verbose=1)
...
d = model.layers[101].get_weights()[0]
print(d.shape)

d = model.layers[101].get_weights()[1]
print(d.shape)
