...
def runCNN(train_indicator=1):
    ...
    state_dim = 192  # Number of weight features
    ...
    for i in range(episode_count):
        ...
        # Instantiate CNN running on MNIST/CIFAR environment @ every episode
        env = CNNEnv()

        s_t, tot_iter, nb_epoch = env.reset(episodes)  # Returns (12, 16) array
        ...
        for j in range(max_steps):
            ...
            s_t1, r_t, done, test_err, train_err, train_loss = env.step(a_t, num_episode)
            ...
            if iter_cnn == tot_iter:
                training_err_graph.append(train_err)
                testing_err_graph.append(test_err)
                training_loss_graph.append(train_loss)

            # Append action/lr at every iteration
            lrate_graph.append(math.exp(a_t[0][0] + a_t[0][1]))
    ...
    training_err_graph = np.asarray(training_err_graph)
    hkl.dump(training_err_graph, 'train_err.hkl', mode='w')

    testing_err_graph = np.asarray(testing_err_graph)
    hkl.dump(testing_err_graph, 'test_err.hkl', mode='w')
