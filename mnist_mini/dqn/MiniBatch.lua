...
function minibatch:reset(batchSize)
    ...
    self.c = {}
    for i=1,10 do
        self.c[i] = torch.load('index/MNIST'..i) --TODO
    end
    self.mapping = {}
    ...
    self.mapping_file = 'logs/mapping.txt'
    self.mapping_file_writer = io.open(self.mapping_file, 'w+')
...
function minibatch:step(action)
    ...
    for i=1,self.rnum do
        if (self.rouletee[i]) then
            ...
            mapp = mapp .. self.rouletee[i] .. " "
        end
    end
    self.mapping_file_writer:write(mapp .. '\n')
    --os.execute('echo ' .. mapp .. ' >> ' .. self.mapping_file)
    local res = {}
    io.flush()
